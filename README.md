...
- **Agent 管理**：检测和管理 Codex、Claude、Claude Code、Claude Desktop Cowork、Cursor、Trae、OpenCode 以及自定义技能目录。Agent 预览会展示已安装和缺失的 Skills，可向该 Agent 添加选中的缺失 Skills，也可从该 Agent 删除选中的已安装 Skills。支持为 Agent 添加自定义标签，并在列表、预览、编辑和同步目标选择中按标签展示或筛选；需要时可修复 Claude Desktop Cowork 清单。支持自定义 agents 配置文件路径。
