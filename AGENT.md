...
└── src-tauri/
    ...
    └── src/
        ...
        ├── mcp_adapter.rs         # MCP 适配器接口（McpAdapter trait）
        ├── mcp_codex.rs           # Codex MCP 适配器（TOML）
        ├── mcp_claude.rs          # Claude Code MCP 适配器（JSON）
        ├── mcp_opencode.rs        # OpenCode MCP 适配器（JSON）
        ├── mcp_trae.rs            # Trae MCP 适配器（JSON）
        ├── mcp_service.rs         # MCP 服务层（McpService）
...
- **MCP（McpView）**：MCP server 管理视图。指标卡（MCP Servers 总数、总配置数、有禁用项）、可滚动 MCP server 列表。每个 server 行显示名称、transport 类型 badge（STDIO/HTTP/SSE）、各 Agent 覆盖状态（彩色圆点：绿色=已配置、黄色=已禁用、灰色=未配置）。点击 server 打开同步弹窗：左侧展示配置详情和原始配置，右侧提供 Agent 勾选列表（默认勾选已配置 Agent）和冲突策略选择。底部按钮支持"同步"（勾选的 Agent）、"同步并清理"（部分取消勾选）和"全部删除"（全部取消勾选）。每个已配置 Agent 旁有禁用/启用按钮。支持添加、编辑操作。添加/编辑表单分两种类型：stdio（command + args + env）和 http/sse（url + headers）。
...
- `scan_mcp_servers()`：扫描所有 Agent 的 MCP server 配置，按名称分组返回。
- `add_mcp_server(agent_ids, config, conflict_policy)`：添加 MCP server 到指定的 agents。
- `update_mcp_server(agent_id, original_name, config)`：更新指定 agent 的 MCP server 配置。
- `remove_mcp_server(agent_id, name)`：从指定 agent 删除 MCP server。
- `toggle_mcp_server(agent_id, name, disabled)`：禁用/启用指定 agent 的 MCP server。
- `sync_mcp_server(server_name, source_agent_id, target_agent_ids, conflict_policy)`：从源 Agent 读取 MCP server 配置，同步到目标 Agents（已存在时按冲突策略处理）。
- `remove_mcp_server_from_agents(server_name, agent_ids)`：批量从多个 Agent 删除 MCP server。

## 后端模块

- `main.rs`：应用入口，调用 `run()` 启动 Tauri + React WebView。
- `lib.rs`：模块导出，提供 `run()` 入口。
- `service.rs`：`AppService` 业务逻辑层，封装 store + adapter，提供所有业务操作。持有 `McpService` 实例。
...
- `mcp_adapter.rs`：MCP 适配器接口（`McpAdapter` trait），定义 scan/add/update/remove/toggle/backup 方法。
- `mcp_codex.rs`：Codex MCP 适配器，读写 `%USERPROFILE%\.codex\config.toml`。
- `mcp_claude.rs`：Claude Code MCP 适配器，读写 `~/.claude.json`，保守处理未知字段。
- `mcp_opencode.rs`：OpenCode MCP 适配器，读写 `~/.opencode.json`。OpenCode 使用 `"mcp"` key、`"local"`/`"remote"` type、`"environment"` env key、`"enabled"` 字段。
- `mcp_trae.rs`：Trae MCP 适配器，读写 `~/.trae/mcp.json`。标准 `mcpServers` JSON 格式，与 Claude 格式一致。
- `mcp_service.rs`：MCP 服务层（`McpService`），聚合所有 MCP 适配器，提供统一的 MCP 管理操作。支持扫描、添加、更新、删除、禁用/启用、同步（从源 Agent 到目标 Agents）和批量删除。
...
- `models.rs`：Rust 侧数据模型，serde 使用 camelCase。包含 MCP 相关类型：`McpTransport`、`McpServerConfig`、`AgentMcpServer`、`GroupedMcpServer`、`McpOperationResult`。
- `error.rs`：统一 `AppError`，可序列化为 Tauri 前端可接收的字符串。包含 `Toml` 变体。
...
- `AgentType` 枚举：Codex / Claude / ClaudeCode / Cursor / Trae / Custom / CherryStudio / OpenCode。
