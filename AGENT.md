...
- 本地状态用 JSON 文件保存（`state.json`），包括主仓库路径、手动添加的 agents、已安装 fingerprint、操作历史和备份位置。
...
- 后端：Rust 2021、serde/serde_json/serde_yaml、walkdir、sha2、chrono、dirs、thiserror、zip。
...
- `store.rs`：JSON 文件持久化层，状态存储在 `state.json` 中。
